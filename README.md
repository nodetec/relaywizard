...
```bash
gpg --verify rwz-0.3.0-alpha5-manifest.sha512sum.asc
```

You should see output similar to the following if the verification was successful:

```bash
gpg: assuming signed data in 'rwz-0.3.0-alpha5-manifest.sha512sum'
gpg: Signature made Mon 05 May 2025 05:57:04 PM UTC
...
```bash
sha512sum --check rwz-0.3.0-alpha5-manifest.sha512sum
```

If the verification was successful you should see the output similar to the following:

```bash
rwz-0.3.0-alpha5-x86_64-linux-gnu.tar.gz: OK
```
